...
const Overview: FC = (): JSX.Element => {
  ...
  // const defaultLength = window.innerWidth <= 500 ? 18 : 23
                  ...
                  <Tooltip text={editTitle || ''}>
                    ...
                    >
                      <AutosizeInput
                        ...
                        defaultValue={
                          editTitle.length >= 22 ? `${editTitle.slice(0, 10)}...` : editTitle || ''
                          // editTitle || ''
                        }
                        onBlur={(e) => {
                          ...
                          editTitle.length >= 22
                            ? (e.target.value = `${editTitle.slice(0, 10)}...`)
                            : editTitle || ''
                          if (editTitle?.length === 0) return dispatch(setEditProjectTitle(title))
                          if (editTitle !== title) return onSaveBlur(e)
                          if (editTitle === title && editTitle.length >= 22)
                            return dispatch(setEditProjectTitle(title))
                        }}
                        ...
                        className={`
                          w-full max-w-[300px] flex-shrink-0 resize-none overflow-hidden text-ellipsis rounded-md border
                          border-transparent py-2 px-4 pr-8 text-4xl font-bold transition duration-150 ease-in-out 
              ...
              {projectStateUpdate ? (
                ...
                <label className="group relative flex items-start md:-mx-2" htmlFor="description">
                  <ReactTextareaAutosize
                    ...
                    className={`
                        min-h-[140px] w-full resize-none break-words rounded-md border-transparent px-4 py-3 pr-10 text-sm
